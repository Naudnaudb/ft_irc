...
void IrcServer::part_command(user &current_user, const std::string &channel_name)
{
	...
	std::string formatted_message = ":" + current_user.username + "!" + current_user.nickname + "@" + SERVER_NAME + " PART " + channel_name;
	send_message_to_channel(channel_name, formatted_message);
}
...
void IrcServer::nick_command(user &current_user, const std::string &nickname)
{
	...
	for (it = users_list.begin(); it != users_list.end(); ++it)
	{
		...
		if (user_checked.nickname == new_nickname && it->first != current_user.socket)
		{
			new_nickname += '_';
			for (it = users_list.begin(); it != users_list.end(); ++it)
			{
	...
	if (old_nickname.empty())
		send_message_to_client(current_user.socket, ":" + std::string(SERVER_NAME) + " 001 " + new_nickname + " :Welcome to the server " + SERVER_NAME + ", " + new_nickname + " !");
	else
		send_message_to_client(current_user.socket, ":" + std::string(SERVER_NAME) + " 001 " + new_nickname + " :Your nickname has change from " + old_nickname + " to " + new_nickname);
}